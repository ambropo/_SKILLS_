...
description: Citation accuracy checker for LaTeX or markdown manuscripts. Reads a .tex or .md file, extracts claim-citation pairs, classifies each claim (FACTUAL/FINDING/METHOD/GENERAL), resolves DOIs via CrossRef fuzzy match, checks Zotero for local copies, fetches abstracts via CrossRef/Semantic Scholar/Scopus/OpenAlex, verifies whether cited papers actually support the claims, and suggests better references where appropriate. Use when the user wants to check citations, verify references, audit a paper's bibliography against its claims, or asks "do my citations actually support what I wrote". Also triggers on "check my citations", "verify references", "citation audit", or similar.
